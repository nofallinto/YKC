...
2025.12.18
1，履带电机pid算法优化，主要是应对履带电机打滑的问题（王仁飞）
2，风机pid算法优化，主要是风机启动慢，导致pid疯狂的增加或减少占空比，导致风机忽大忽小（王仁飞）
3，优化陀螺仪驱动，补偿零漂的Gy计算Pitch改变（王仁飞）
4，修复GPRS通信时的环形buff头尾指针更新有误（李坤鹏）
5，新增风机完全停止计时器，等风机完全停住时才记录空闲时气压（李坤鹏）
		  		
2025.11.20
1，串口收发改用可选择使用DMA模式还是普通模式（李坤鹏）
2，GPRS兼容4G模块，同时GPRS相关通信统一使用GPRS_SendATCmd函数。网络通信recv、send等相关函数优化（李坤鹏）
3，新增陀螺仪中断判断堵转、陀螺仪角速度长时间不变判断堵转（李坤鹏）
4，履带增加限速，擦窗机路径优化，新增“探索上边沿”动作（李坤鹏）
5，陀螺仪计算相关bug修改，主要是计算时用的量程与配置的不一样（李坤鹏）
