...
#define MQTT_WILL_QOS      	1<<3	/*3 and 4*/
...
//		}
//		broker->seq++;
//		/* 孟尧尧添加，理由：这个是发布端产生的消息ID， 它要和服务器端的消息ID一直，服务器端的消息在到达256以后就会置零。如果发布端消息ID布置零，就会退出发布任务*/
//		//If the number of send bars is greater than 256, the ID is set to zero
//		if(broker->seq == 256) {
//			broker->seq = 0;
//		}
//	}
...
//
//	// Send the packet
//	if(broker->send(broker->i32Socket, packet, sizeof(packet)) < sizeof(packet)) {
//		return -1;
//	}
//
//	return 1;
