...
const uint8 cnst_u8Bmi270InitConf[8192];

BOOL WriteDatToBmi270(uint8 u8DataAddr, const uint8* pU8, uint16 uBLen);
inline BOOL WriteDatToBmi270(uint8 u8DataAddr, const uint8* pU8, uint16 uBLen)
{
    BMI270_CS(0);
    BOOL bRes = WriteToSPI(SPI_PORT_BMI270, &u8DataAddr, 1, 0)
                && WriteToSPI(SPI_PORT_BMI270, pU8, uBLen, 0);
    BMI270_CS(1);
    return bRes;
}

BOOL ReadDatFromBmi270(uint8 u8DataAddr, uint8* pU8, uint16 uBLen);
inline BOOL ReadDatFromBmi270(uint8 u8DataAddr, uint8* pU8, uint16 uBLen)
{
    BMI270_CS(0);
    BOOL bRes = WriteToSPI(SPI_PORT_BMI270, &u8DataAddr, 1, 0)
                && ReadFromSPI(SPI_PORT_BMI270, pU8, uBLen, 0);
    BMI270_CS(1);
    return bRes;
}

/*==========================================================================
| Description	: BMI270 自检
| In/Out/G var	: void
| 返回参数        BOOL           TRUE自检成功, FALSE自检失败
| Author		: Cui				Date	: 2024-01-27
\=========================================================================*/
static BOOL Bmi270SelfCheck(void)
{
    uint8 u8TryCount = 0;
    for( ;u8TryCount < BMI270_TIMEOUT_COUNT; u8TryCount++) {
    	uint8 u8Buf[2] = {0, 0};
        ReadDatFromBmi270(BMI270_CHIP_ID | BMI270_SPI_R, u8Buf, 2);
        osDelay(OS_TICK_KHz * 1);
    	if(0x24 == u8Buf[1]) {		/* 读取设备ID是否等于0x24，如果不是0x24则认为没检测到设备 */
    		return TRUE;
    	}
    }
    return FALSE;
...
/*==========================================================================
| Description	: 获取 BMI270 加速度计数据
| In/Out/G var	: void
| 				（SPI读取第一个地址为空）
|					使用 SPI 的采集时间为69us ，采集加速度计的时间与采集陀螺仪的时间一致的原因是都只是读取寄存器数据
| Author		: Cui				Date	: 2024-01-27
\=========================================================================*/
BOOL Bmi270GetAcc(void)
{
	uint8 u8Buf[7] = {0,0,0,0,0,0,0};
	/* SPI读取第一个地址为空 */
	if(ReadDatFromBmi270(BMI270_ACC_ADDRESS | BMI270_SPI_R, u8Buf, 7)) {
		g_Bmi270Comm.iAccX = (int16)(((uint16)u8Buf[2]<<8) | u8Buf[1]);
		g_Bmi270Comm.iAccY = (int16)(((uint16)u8Buf[4]<<8) | u8Buf[3]);
		g_Bmi270Comm.iAccZ = (int16)(((uint16)u8Buf[6]<<8) | u8Buf[5]);
		return TRUE;
	} else {
		return FALSE;
	}
...
/*==========================================================================
| Description	: 获取 BMI270 陀螺仪数据
| In/Out/G var	: void
| 				（SPI读取第一个地址为空）
|					使用 SPI 的采集时间为69us ，采集加速度计的时间与采集陀螺仪的时间一致的原因是都只是读取寄存器数据
| Author		: Cui				Date	: 2024-01-27
\=========================================================================*/
BOOL Bmi270GetGyro(void)
{
	uint8 u8Buf[7] = {0,0,0,0,0,0,0};
	/* SPI读取第一个地址为空 */
	if(ReadDatFromBmi270(BMI270_GYRO_ADDRESS | BMI270_SPI_R, u8Buf, 7)) {
		g_Bmi270Comm.iGyrX = (int16)(((uint16)u8Buf[2]<<8) | u8Buf[1]);
		g_Bmi270Comm.iGyrY = (int16)(((uint16)u8Buf[4]<<8) | u8Buf[3]);
		g_Bmi270Comm.iGyrZ = (int16)(((uint16)u8Buf[6]<<8) | u8Buf[5]);
		return TRUE;
	} else {
		return FALSE;
	}
...
/*==========================================================================
| Description	: 将 BMI270 加速度计数据转换为实际物理数据
| In/Out/G var	: iAccValue 任意轴的加速度计数据
| 使用示例         float32 data = Bmi270AccTransition(bmi270_acc_x);  //单位为 g(m/s^2)
| Author		: Cui				Date	: 2024-01-27
\=========================================================================*/
float32 Bmi270AccTransition(int16 iAccValue)
{
    float32 fAccData = 0;
    switch(BMI270_ACC_SAMPLE) {
        /* 0x00 加速度计量程为:±2g  获取到的加速度计数据 除以16384 可以转化为带物理单位的数据，单位：g(m/s^2)*/
        case 0x00: fAccData = (float32)iAccValue / 16384; break;
        /* 0x01 加速度计量程为:±4g  获取到的加速度计数据 除以8192  可以转化为带物理单位的数据，单位：g(m/s^2)*/
        case 0x01: fAccData = (float32)iAccValue / 8192; break;
        /* 0x02 加速度计量程为:±8g  获取到的加速度计数据 除以4096  可以转化为带物理单位的数据，单位：g(m/s^2)*/
        case 0x02: fAccData = (float32)iAccValue / 4096; break;
        /* 0x03 加速度计量程为:±16g 获取到的加速度计数据 除以2048  可以转化为带物理单位的数据，单位：g(m/s^2)*/
        case 0x03: fAccData = (float32)iAccValue / 2048; break;
        default: break;
    }
    return fAccData;
}
...
/*==========================================================================
| Description	: 将 BMI270 陀螺仪数据转换为实际物理数据
| In/Out/G var	: iGyroValue 任意轴的陀螺仪数据
| 使用示例         float32 data = Bmi270GyroTransition(bmi270_acc_x);  //单位为°/s
| Author		: Cui				Date	: 2024-01-27
\=========================================================================*/
float32 Bmi270GyroTransition(int16 iGyroValue)
{
    float32 fGyroData = 0;
    switch(BMI270_GYR_SAMPLE) {
        /*  0x00 陀螺仪量程为:±2000dps  获取到的陀螺仪数据除以16.4  可以转化为带物理单位的数据，单位为：°/s */
        case 0x00: fGyroData = (float32)iGyroValue / 16.4f;	break;
        /*  0x01 陀螺仪量程为:±1000dps  获取到的陀螺仪数据除以32.8  可以转化为带物理单位的数据，单位为：°/s */
        case 0x01: fGyroData = (float32)iGyroValue / 32.8f;	break;
        /*  0x02 陀螺仪量程为:±500 dps  获取到的陀螺仪数据除以65.6  可以转化为带物理单位的数据，单位为：°/s */
        case 0x02: fGyroData = (float32)iGyroValue / 65.6f;	break;
        /*  0x03 陀螺仪量程为:±250 dps  获取到的陀螺仪数据除以131.2 可以转化为带物理单位的数据，单位为：°/s */
        case 0x03: fGyroData = (float32)iGyroValue / 131.2f; break;
        /*  0x04 陀螺仪量程为:±125 dps  获取到的陀螺仪数据除以262.4 可以转化为带物理单位的数据，单位为：°/s */
        case 0x04: fGyroData = (float32)iGyroValue / 262.4f; break;
        default: break;
    }
    return fGyroData;
}
...
BOOL Bmi270Init(void)
{
	...
	if(Bmi270SelfCheck()) {                                           		/* BMI270自检 */
		u8Buf[0] = 0x00;
		WriteDatToBmi270(BMI270_PWR_CONF | BMI270_SPI_W, u8Buf, 1);         /* 关闭高级省电模式 */
		osDelay(OS_TICK_KHz * 1);
		u8Buf[0] = 0x00;
		WriteDatToBmi270(BMI270_INIT_CTRL | BMI270_SPI_W, u8Buf, 1);        /* 开始对模块进行初始化配置 */
		WriteDatToBmi270(BMI270_INIT_DATA | BMI270_SPI_W, cnst_u8Bmi270InitConf, sizeof(cnst_u8Bmi270InitConf));	/* 输出配置文件 */
		u8Buf[0] = 0x01;
		WriteDatToBmi270(BMI270_INIT_CTRL | BMI270_SPI_W, u8Buf, 1);         /* 初始化配置结束 */
		osDelay(OS_TICK_KHz * 20);
...
//			u8Buf[0] = (1 << 7) | (1 << 6) | (0x00 << 4) | 0x08;		/* 0xA9 */
			...
			/* 配置初始化文件出错了, 检查一下接线有没有问题 如果没问题可能就是坏了 */
		}
	} else {
		/* 配置初始化文件出错了, 检查一下接线有没有问题 如果没问题可能就是坏了 */
	}
